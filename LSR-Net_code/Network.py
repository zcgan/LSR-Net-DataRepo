...
class FourierLayer(nn.Module):
    def __init__(self, fourier_channels, initial_amplitude=None, initial_shift=None, initial_beta=None, phase=1):
        super(FourierLayer, self).__init__()
        self.num_channels = fourier_channels
        self.L = 16*np.pi
    ...
    def forward(self, x):
        ...
        k = torch.sqrt(kx_grid ** 2 + ky_grid ** 2).to(x.device) / x.shape[2]

    

        multipliers = []
        for i in range(self.num_channels):
            part1 = self.amplitude[i] / ((k+ self.shift[i]) ** 2 + self.amplitude[i] ** 2)
            part2 = self.amplitude[i] / ((k - self.shift[i]) ** 2 + self.amplitude[i] ** 2)
            filter_k = k**2 / (k**2  + 400*self.hypera[i]**2)
...
class SpectralLayer(nn.Module):
    def __init__(self, in_channels, out_channels, channel_feat, kernel_size,
                 fourier_channels=None, initial_amplitude=None, initial_shift=None,
        ...
        self.fu = FourierLayer(self.fourier_channels, initial_amplitude, initial_shift, initial_beta, phase=phase)
        pad = (kernel_size - 1) // 2
...
class Wise_SpectralTransform(nn.Module):
    def __init__(self, num_layers, channel_feat, kernel_size,
                 fourier_channels=None, phase=1, initial_parameters=None):
